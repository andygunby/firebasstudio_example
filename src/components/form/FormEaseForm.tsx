  ...
  if (step === 'success' && submittedData) {
    return (
        <div className="w-full max-w-lg text-center bg-card p-8 rounded-xl border">
            ...
            <p className="text-muted-foreground">Thank you for submitting your details.</p>
        ...
        <div>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                /> }
                <Button type="submit" className="w-full" size="lg" disabled={isSubmitting || isExtracting}>
                  {(isSubmitting || isExtracting) && <Loader2 className="animate-spin" />}
                  {isSubmitting ? 'Submitting...' : isExtracting ? 'Analyzing...' : 'Submit'}
                </Button>
              </form>
            </Form>
        </div>
